...
def process_incoming_data(user_id, ydx_server, ydx_app_host, ai_user_id, youtube_id):
    ...
    try:
        ...
        logger.info(f"Successfully processed incoming data for video {youtube_id}")
    except sqlite3.Error as e:
...
def get_status_for_youtube_id(youtube_id, ai_user_id):
    try:
        ...
        with connection.return_connection() as con:
            cursor = con.cursor()
            logger.info(
                f"Executing query with youtube_id: {youtube_id}, ai_user_id: {ai_user_id} (types: {type(youtube_id)}, {type(ai_user_id)})")
            cursor.execute('''
...
def update_status(youtube_id: str, ai_user_id: str, status: str):
    try:
        ...
        if youtube_id is None or ai_user_id is None or status is None:
            logger.error(
                f"Cannot update status with None values. YouTube ID: {youtube_id}, AI User ID: {ai_user_id}, Status: {status}")
            return
        ...
        logger.error(
            f"SQLite error updating status for YouTube ID: {youtube_id}, AI User ID: {ai_user_id}, Status: {status}")
...
def update_ai_user_data(youtube_id, ai_user_id, user_id, status):
    ...
    try:
        with connection.return_connection() as con:
            ...
            logger.info(f"Updated AI user data status for YouTube ID: {youtube_id}, user ID: {user_id}")
    except sqlite3.Error as e:

...
warnings.filterwarnings("error")  # Catch warnings and turn them into exceptions for strict error handling
...
def cosine_similarity(v1, v2):
    ...
    try:
        ...
        return "NaN"  # Return NaN to signal similarity could not be calculated
...
def generate_average_output(video_runner_obj: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        captions_and_objects_csv = os.path.join(return_video_folder_name(video_runner_obj), CAPTIONS_AND_OBJECTS_CSV)
        output_avg_csv = os.path.join(return_video_folder_name(video_runner_obj), OUTPUT_AVG_CSV)
        ...
        if not jsonArray:  # Check if the input CSV is empty
            logger.error(f"Input file {captions_and_objects_csv} is empty.")
            return False

        # Initialize lists for storing data
        isKeyFrame, description, frame_index, timestamp, list_of_vectors = [], [], [], [], []

        # Process each row in the CSV and extract necessary data
        ...
        # Generate similarity data based on vectors
        for idx in range(2, len(list_of_vectors) - 1):
            s = return_int_if_possible(cosine_similarity(list_of_vectors[idx], list_of_vectors[idx + 1]))
            a1 = return_int_if_possible(
                cosine_similarity(list_of_vectors[idx - 1], list_of_vectors[idx + 2])) if idx < len(
                list_of_vectors) - 3 else 0.0
            a2 = return_int_if_possible(
                cosine_similarity(list_of_vectors[idx - 2], list_of_vectors[idx + 3])) if idx < len(
                list_of_vectors) - 3 else 0.0
            s = "SKIP" if s == "NaN" else s  # Mark "NaN" as "SKIP"
        ...
        try:
            with open(output_avg_csv, 'w', newline='') as csvFile:
                ...
                writer.writerow(['frame', 'timestamp', 'Line1', 'Line2', 'Similarity', 'avgone', 'avgtwo', 'isKeyFrame',
                                 'description'])
                writer.writerows(data)
        except Exception as e:
            logger.error(f"Failed to write output file: {output_avg_csv}, Error: {str(e)}")
            return False
        ...
        logger.info(f"Output avg CSV file successfully generated for video: {video_id}")
